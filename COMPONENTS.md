...
- `RecordBook` — The league's record book from the matchups feed of every season: the ten highest team weeks ever (opponent, margin, playoff marked), the same for one week number across seasons (-Pweek), each manager's best week, the ten lowest weeks that had a game (no-game rows counted apart), what carried the top ten (best starter's and best three's share of the team total, against an ordinary week), and the ten highest weeks by a started player. The open week is excluded unless -Plive. Report to data/record-book-<date>.txt.
...
- `RosModel` — A rest-of-season rate after k games, three ways on squared error: the posterior, Sleeper's week k+1 projection, and their least-squares blend, weight fitted leave-one-season-out over every season Sleeper serves week projections for (2018 on), on the whole preseason board (every rostered man is on it), read per position and k against Student's t for the seasons. THIS WEEK applies it to the season being played; -Pprojections=ros serves it to every tool as the rate times the weeks Sleeper projects him to play. ESPN's weekly projections (EspnWeekly, 2018 on) are tested as a third estimator on the same men: on 2026-09-25 they earned a place at 1 of 6 weeks-seen, noise on that evidence, so the source stays two-way. Report to data/ros-model-<date>.txt.
