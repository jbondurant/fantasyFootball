...
- `RecordBook` — The league's record book from the matchups feed of every season: the ten highest team weeks ever (opponent, margin, playoff and no-game marked), the same for one week number across seasons (-Pweek), each manager's best week, and the ten highest weeks by a started player. The current week is excluded until it is over. Report to data/record-book-<date>.txt.
