...
./gradlew run -Pmain=TuesdaySwap         # IN SEASON: every (add, drop) pair on the weekly-starter objective; DO NOTHING is the default answer (-Pcandidates -PswapFloor -Pscenarios; -Pprojections=posterior prices on Sleeper's numbers moved by the played weeks, report to data/tuesday-swap-<season>-w<week>-posterior.txt)
./gradlew run -Pmain=RecordBook          # the league's record book: the ten highest team weeks ever, the ten highest for one week number across seasons (-Pweek=n), each manager's best week, the ten highest started-player weeks (-Ptop; -Plive also ranks the week Sleeper has not closed, every row marked partial)
./gradlew run -Pmain=WaiverLog           # IN SEASON: every waiver claim this season, won and failed, with bids by manager, the clearing price, FAAB spent per manager against Sleeper's own counter, and my own claims
